...
What if we want to observe the relationship between two categorical variables?  

In this case, we will examine the number of penguins of each species originating from each island. We can do so using a **grouped bar plot**. This is particularly straightforward to implement using `ggplot2`, but we will need to use `dplyr` to construct a summary tally of penguins by species and island and use that to make the graph.
...
What do you think the code below does differently than the previous example?
...
# Practice: Data visualization
  ...
  1. Create a new variable in the dataset to represent the ratio of bill length to depth. Plot your new variable in a boxplot and change the color of the inside of the boxplot. HINT: see `?boxplot` or `?geom_boxplot`.
  2. Create scatter plots in both base `R` and `ggplot2` of flipper length against bill length that uses symbols to indicate island of observation and color to indicate species. 
  ...
  4. Use `group_by()` + `summarize()` or `aggregate()` to get the number of observed penguins by island, species, and year. Use `geom_bar()` and `facet_wrap()` to create a 3-panel plot (one for each island), that displays the penguin observation count by year, with color indicating species.
